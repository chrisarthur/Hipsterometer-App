    ...
    private class TsunamiAsyncTask extends AsyncTask<URL, Void, Event> {
        ...
        private String makeHttpRequest(URL url) throws IOException {
            ...
            try {
                ...
                // If successful, parc JSON response
                int httpResponse = urlConnection.getResponseCode();
                if (httpResponse == 200) {
                    inputStream = urlConnection.getInputStream();
                    jsonResponse = readFromStream(inputStream);
                } else {
                    Log.e(LOG_TAG, "error response code: " + Integer.toString(httpResponse));
                }
            } catch (IOException e) {
                Log.e(LOG_TAG, "IOException in sending http GET request", e);
